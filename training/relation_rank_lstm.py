...
class ReRaLSTM:
    def __init__(self, data_path, market_name, tickers_fname, relation_name,
        ...
        relation_file = os.path.join(PROJECT_ROOT, 'relation', self.relation_name,
                                     self.market_name + rname_tail[self.relation_name])

        print(f"Loading relation data from: {relation_file}")  # 添加调试信息
        if not os.path.exists(relation_file):
            raise FileNotFoundError(f"Relation file not found at: {relation_file}")

        self.rel_encoding, self.rel_mask = load_relation_data(relation_file)
        ...
        # 使用PROJECT_ROOT (确保之前已经定义了PROJECT_ROOT)
        # 使用PROJECT_ROOT构建embedding文件路径
        embedding_file = os.path.join(PROJECT_ROOT, 'data', 'pretrain', emb_fname)
        print(f"Loading embedding from: {embedding_file}")  # 添加调试信息
        ...
        self.valid_index = int(self.mask_data.shape[1] * 0.6)  # 60% 用于训练
        self.test_index = int(self.mask_data.shape[1] * 0.8)  # 20% 用于验证，20% 用于测试
        ...
        print("Data shapes:")
        print("- embedding shape:", self.embedding.shape)
        print("- eod_data shape:", self.eod_data.shape)
        print("- mask_data shape:", self.mask_data.shape)
    ...
    #         )
    def get_batch(self, offset=None):
        """Get a batch of data for training/testing

        Args:
            offset: starting position of the batch. If None, randomly select a position

        Returns:
            tuple: (embedding, mask_batch, price_batch, ground_truth_batch)
        """
        # 打印调试信息
        # print("Debug - Embedding shape:", self.embedding.shape)
        # print("Debug - Mask data shape:", self.mask_data.shape)
        # print("Debug - Price data shape:", self.price_data.shape)
        # print("Debug - GT data shape:", self.gt_data.shape)
        ...
        # 确保索引不会超出范围
        max_offset = min(offset + seq_len + self.steps, self.mask_data.shape[1])
        mask_batch = self.mask_data[:, offset:max_offset]
        mask_batch = np.min(mask_batch, axis=1)

        # 根据 embedding 的维度决定如何获取数据
        if len(self.embedding.shape) == 2:  # 如果是 (num_stocks, feature_dim)
            emb_batch = self.embedding
        elif len(self.embedding.shape) == 3:  # 如果是 (num_stocks, timesteps, feature_dim)
            emb_batch = self.embedding[:, offset, :]
        else:
            raise ValueError(f"Unexpected embedding shape: {self.embedding.shape}")

        # 确保价格和ground truth的索引不会超出范围
        price_idx = min(offset + seq_len - 1, self.price_data.shape[1] - 1)
        gt_idx = min(offset + seq_len + self.steps - 1, self.gt_data.shape[1] - 1)

        # 构建返回数据
        try:
            price_batch = self.price_data[:, price_idx]
            gt_batch = self.gt_data[:, gt_idx]

            return (emb_batch,
                    np.expand_dims(mask_batch, axis=1),
                    np.expand_dims(price_batch, axis=1),
                    np.expand_dims(gt_batch, axis=1))

        except IndexError as e:
            print(f"Error occurred when getting batch with offset {offset}:")
            print(f"- Price index: {price_idx}")
            print(f"- GT index: {gt_idx}")
            print(f"- Data shapes:")
            print(f"  * Embedding: {self.embedding.shape}")
            print(f"  * Mask: {self.mask_data.shape}")
            print(f"  * Price: {self.price_data.shape}")
            print(f"  * GT: {self.gt_data.shape}")
            raise e
    ...
    def train(self):
        ...
        with tf.device(device_name):
            ...
            if self.inner_prod:
                ...
                #                               activation=leaky_relu)
                head_weight = tf.keras.layers.Dense(units=1, activation=leaky_relu)(feature)
                tail_weight = tf.keras.layers.Dense(units=1, activation=leaky_relu)(feature)
            ...
            prediction = tf.keras.layers.Dense(units=1, activation=leaky_relu, name='reg_fc',
                                               kernel_initializer=tf.glorot_uniform_initializer())(outputs_concated)
...
if __name__ == '__main__':
    ...
    #                     default='../data/pretrain/NASDAQ_rank_lstm_seq-8_unit-32_0.csv.npy',
    #                     help='fname for pretrained sequential embedding')
    parser.add_argument('-e', '--emb_file', type=str,
                        default='NASDAQ_rank_lstm_seq-8_unit-32_0.csv.npy',
                        help='fname for pretrained sequential embedding')
    ...
    if args.t is None:
        args.t = '../data/NASDAQ_tickers_valid.csv'
    args.gpu = (args.gpu == 1)
